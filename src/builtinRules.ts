...
import { MergedReviewItem } from './types';
...
function lineToItem(
  line: number,
  category: MergedReviewItem['category'],
  message: string,
...
const SECURITY_PATTERNS: { pattern: RegExp; message: string; severity?: 'error' | 'warning' }[] = [
...
export function runSecurityRules(document: vscode.TextDocument): MergedReviewItem[] {
  console.log('Running security rules...');
  const config = vscode.workspace.getConfiguration('smartCodeReview.rules');
  const enableBuiltin = config.get<boolean>('enableBuiltin', true);
  console.log('enableBuiltin:', enableBuiltin);
  if (!enableBuiltin) return [];
  ...
  lines.forEach((line, i) => {
    ...
    for (const { pattern, message, severity } of SECURITY_PATTERNS) {
      if (pattern.test(line)) {
        ...
        if (lang === 'python' && /document\.|innerHTML|new\s+Function/.test(pattern.source)) continue;
        if ((lang === 'javascript' || lang === 'typescript') && /pickle|__import__|os\.system|subprocess/.test(pattern.source)) continue;
...
/** 性能：按行检测常见低效写法 */
export function runPerformanceRules(document: vscode.TextDocument): MergedReviewItem[] {
...
/** 重构：长函数、重复块 */
export function runRefactorRules(
  ...
  if (ir) console.log('Functions:', ir.functions.length);
      ...
      if (lineCount > MAX_FUNCTION_LINES) {
        items.push(
          lineToItem(
            fn.startLine,
            'refactor',
            `函数 "${fn.name}" 过长（${lineCount} 行），建议拆分为更小函数或提取子逻辑`,
            'info'
          )
        );
      }
      ...
      if (block === other) {
        items.push(
          lineToItem(
            i + 1,
            'refactor',
            `第 ${i + 1}–${i + DUPLICATE_BLOCK_MIN_LINES} 行与第 ${j + 1}–${j + DUPLICATE_BLOCK_MIN_LINES} 行重复，建议提取为公共函数或常量`,
            'info'
          )
        );
        break;
