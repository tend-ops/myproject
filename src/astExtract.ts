...
import { CodeIntermediateRepresentation, FunctionNode, VariableNode, ControlNode } from './types';
...
function getNodeText(node: { startIndex: number; endIndex: number }, source: string): string {
  return source.slice(node.startIndex, node.endIndex);
}

function findChild(node: { childCount: number; child: (i: number) => any }, ...types: string[]): any {
...
function findIdentifierName(node: any, source: string): string | undefined {
...
function extractPython(tree: { rootNode: any }, source: string): Omit<CodeIntermediateRepresentation, 'languageId' | 'filePath' | 'contentHash'> {
  const functions: FunctionNode[] = [];
  const variables: VariableNode[] = [];
  const controlStructures: ControlNode[] = [];
  const errors: string[] = [];
...
function extractJavaScript(tree: { rootNode: any }, source: string): Omit<CodeIntermediateRepresentation, 'languageId' | 'filePath' | 'contentHash'> {
  const functions: FunctionNode[] = [];
  const variables: VariableNode[] = [];
  const controlStructures: ControlNode[] = [];
  const errors: string[] = [];
...
function extractJava(tree: { rootNode: any }, source: string): Omit<CodeIntermediateRepresentation, 'languageId' | 'filePath' | 'contentHash'> {
  const functions: FunctionNode[] = [];
  const variables: VariableNode[] = [];
  const controlStructures: ControlNode[] = [];
  const errors: string[] = [];
...
export function getCodeSnippetAtLine(ir: CodeIntermediateRepresentation, source: string, line: number): string {
